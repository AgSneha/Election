...
contract("Election", function(accounts){
	...
	it("throws an exception for invalid candidates", function(){
		return Election.deployed().then(function(instance){
			...
			assert.equal(voteCount, 1, "candidate 1 did not rcv anny votes");
			return electionInstance.candidates(2);
		}).then(function(candidate2){
			var voteCount = candidate2[2];
			assert.equal(voteCount, 0, "candidate 2 did not rcv any votes");
		});
	});
	...
	it("throws an exception for double voting", function(){
		return Election.deployed().then(function(instance){
			...
			var voteCount = candidate1[2];
			assert.equal(voteCount, 1, "candidate 1 did not rcv any votes");
			return electionInstance.candidates(2);
		}).then(function(candidate2){
			var voteCount = candidate2[2];
			assert.equal(voteCount, 1, "candidate 2 did not rcv any votes");
		});
	});
